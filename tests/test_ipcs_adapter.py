...
def test_named_toxicity_endpoint_and_physchem_split(tmp_path: Path):
    adapter = IPCSAdapter()
    html = """
    <html><body>
      Molecular weight is 84.07 g/mol.
      Melting point 156-159 C.
      LC50 was > 10 mg/litre in fish.
      LOAEL 2 mg/kg body weight in rats.
    </body></html>
    """
    rows = adapter._build_doc_rows(
        query="amitrole",
    ...
    tox_named = [r for r in rows if r.field_name == "toxicity_endpoint_value"]
    assert any(r.endpoint == "LC50" and r.numeric_value == "10" and "mg/litre" in r.unit for r in tox_named)
    assert any(r.endpoint == "LOAEL" and r.numeric_value == "2" and "mg/kg" in r.unit for r in tox_named)

    physchem = [r for r in rows if r.field_name.startswith("physchem_")]
    assert any(r.field_name == "physchem_molecular_weight" and r.numeric_value == "84.07" for r in physchem)
    assert any(r.field_name == "physchem_melting_point" and "156-159" in r.numeric_value for r in physchem)
