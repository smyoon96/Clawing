...
`focused.*`는 핵심 유해성 필드(`ehc_endpoint_measurement`, `toxicity_endpoint_value`, `hazard_code`, `cas_number_detected`, `table_hazard_extract`, `ipcs_reference`, `toxicity_metric`)와 주요 물리화학 필드(`physchem_*`)만 중복 제거해 저장합니다.
