...
```bash
python run_ingestion.py --input-file ./cas_list.xlsx --sources "iris,hcis,atsdr,iarc" --output-dir ./output
```

의존성/네트워크 없이 파이프라인만 검증하려면:

```bash
python run_ingestion.py --input-file ./cas_list.csv --sources "iris,hcis,atsdr,iarc" --output-dir ./output --dry-run
```

프록시 환경이면:

```bash
python run_ingestion.py --input-file ./cas_list.csv --sources "iris,hcis,atsdr,iarc" --output-dir ./output --proxy "http://host:port" --retries 3 --backoff-sec 2
```
...
- 구현 adapter:
    ...
    - `source_adapters/atsdr_adapter.py`
    - `source_adapters/iarc_adapter.py`
