...
class IPCSAdapter(BaseAdapter):
    ...
    PHYS_CHEM_PATTERNS: tuple[tuple[str, str], ...] = (
        ("molecular_weight", r"molecular\s+weight[^\d]{0,20}(\d+(?:[.,]\d+)?)\s*(g/mol)?"),
        ("melting_point", r"melting\s+point[^\d]{0,20}(\d+(?:[.,]\d+)?(?:\s*[-–]\s*\d+(?:[.,]\d+)?)?)\s*(?:°?c|c)"),
        ("boiling_point", r"boiling\s+point[^\d]{0,20}(\d+(?:[.,]\d+)?)\s*(?:°?c|c)"),
        ("vapor_pressure", r"vapou?r\s+pressure[^\d]{0,20}(\d+(?:[.,]\d+)?)\s*(pa|kpa|mmhg)"),
        ("water_solubility", r"soluble\s+in\s+water[^\d]{0,40}(\d+(?:[.,]\d+)?)\s*(mg/l|g/l|g/litre|mg/litre)"),
        ("log_kow", r"log\s*kow[^\d]{0,20}([<>]?\s*\d+(?:[.,]\d+)?)"),
    )
    ...
    @staticmethod
    def _extract_named_metrics(text: str) -> list[tuple[str, str, str, str, str]]:
        out: list[tuple[str, str, str, str, str]] = []
        pat = re.compile(
            r"\b(?P<metric>LD50|LC50|EC50|IC50|NOAEL|NOEL|LOAEL|LOEL|ADI|TDI|BMDL?|TLV|STEL)\b"
            r"[^\.\n]{0,90}?"
            r"(?P<cmp><=|>=|<|>|=)?\s*(?P<num>\d+(?:[.,]\d+)?)\s*(?P<unit>mg/kg(?:\s*body\s*weight)?|mg/litre|g/litre|mg/m3|ppm|ppb|mg/l|g/l|μg/l|ug/l)?",
            flags=re.I,
        )
        for m in pat.finditer(text or ""):
            ...
            raw = m.group(0).strip()
            if num:
                out.append((metric, cmp_, num, unit, raw[:240]))
        return out

    @classmethod
    def _extract_physchem_properties(cls, text: str) -> list[tuple[str, str, str, str, str]]:
        rows: list[tuple[str, str, str, str, str]] = []
        for field, pat in cls.PHYS_CHEM_PATTERNS:
            for m in re.finditer(pat, text or "", flags=re.I):
                num = (m.group(1) or "").replace(",", ".").strip()
                unit = (m.group(2) if (m.lastindex or 0) >= 2 else "") or ""
                unit = unit.strip()
                rows.append((field, "", num, unit, m.group(0).strip()[:240]))
        return rows
    ...
    def _build_doc_rows(
        ...
        *,
        ...
        # endpoint 이름 + value/unit 분리 추출
        for metric, cmp_, num, unit, raw in self._extract_named_metrics(doc_text):
            rows.append(
                UnifiedRow(
                    source_name=self.source_key,
                    query_input=query,
                    ...
                    substance_name=substance_name,
                    endpoint=metric,
                    field_name="toxicity_endpoint_value",
                    raw_value=raw,
                    comparator=cmp_,
                    numeric_value=num,
                    unit=unit,
                    qualifier="",
                    hazard_code="",
                    hazard_category="toxicity",
                    study_guideline="",
                    test_conditions="",
                    section_path="ipcs.document.named_metric",
                    evidence_url=doc_url,
                    evidence_file=str(evidence_file),
                    retrieved_at_utc=self.now_utc_iso(),
                )
        ...
        # 물리화학적 특성 추출
        for field, cmp_, num, unit, raw in self._extract_physchem_properties(doc_text):
            rows.append(
                UnifiedRow(
                    source_name=self.source_key,
                    query_input=query,
                    cas_number="",
                    substance_name=substance_name,
                    endpoint="physchem_properties",
                    field_name=f"physchem_{field}",
                    raw_value=raw,
                    comparator=cmp_,
                    numeric_value=num,
                    unit=unit,
                    qualifier="",
                    hazard_code="",
                    hazard_category="physchem",
                    study_guideline="",
                    test_conditions="",
                    section_path="ipcs.document.physchem",
                    evidence_url=doc_url,
                    evidence_file=str(evidence_file),
                    retrieved_at_utc=self.now_utc_iso(),
                )
